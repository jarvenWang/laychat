...
//充值  提现  账户明细  切换
var accountTabIndex = null,
    accountTabLi = $('.account-tab li'),
    accountContent = $('.account-content');
function withdrawPromise(index){
    if(index==1){
        //提现前提条件：实名认证和资金密码设置
        if(!pangu.myData.name){
            promptBox('请先实名认证！','立即认证',function(index){
                layer.close(index);
                nameTestFn1();
            },function(){
                window.location.href = "memberCenter.html"
            })
        }else if(!pangu.myData.pay_password){
            promptBox('请先设置资金密码！','立即设置',function(index){
                layer.close(index);
                paywordSetFn1();
            },function(){
                window.location.href = "memberCenter.html"
            ...
            //获取银行卡列表
            //getData('api',function(res){
            //    console.log(res);
            //});
            accountTabLi.eq(index).addClass('main-tab-active');
            accountContent.eq(index).show();
        }
...
function accountManageTab(){
    //通过链接跳转到相应tab
    if(!localStorage.accountManageLink){
        accountTabIndex = 0;
    }else{
        accountTabIndex = localStorage.accountManageLink;
    }
    accountTabLi.removeClass('main-tab-active');
    accountContent.hide();
    withdrawPromise(accountTabIndex);

    //点击切换
    accountTabLi.click(function(){
        var tabLiIndex = $(this).index();
        accountTabLi.removeClass('main-tab-active');
        accountContent.hide();
        withdrawPromise(tabLiIndex);
    })
}
...
/********************充值***********************/
    //选择充值活动
var rechargeActiveChoose = 0;
var rechargeActiveNum = [];
    //选择在线支付方式
var rechargeTypesBox1 = $('.online-pay .recharge-types');
var rechargeTypeInput1 = $('.online-pay .recharge-type-input input');
var formTips1 = $('.online-pay .recharge-type-choose .form-tips');
    //选择转账汇款方式
var rechargeTypesBox2 = $('.transfer-accounts .recharge-types');
var rechargeTypeInput2 = $('.transfer-accounts .recharge-type-input input');
var formTips2 = $('.transfer-accounts .recharge-type-choose .form-tips');
    //倒计时
var amountDownTimer = null;
//tab切换
function rechargeTab(){
    ...
    //点击切换充值方式
    var formRecharge = $('.recharge-form');
    $('.recharge-tab li').click(function(){
        var formIndex = $(this).index();
        $('.recharge-tab li div').removeClass('main-sub-tab-active');
        $(this).find('div').addClass('main-sub-tab-active');
        formRecharge.hide();
        formRecharge.eq(formIndex).show();
        //在线支付
        if(formIndex==0){
            onlinePayRecharge();
        }
...
/*****充值活动*****/
//充值活动公告
function showRechargeMessage(){
    var activeMessageUl = $('.recharge-message-form .main-form-item-r');
    var activeMessageLi = '';
    postData('recharge-active',{'token':localStorage.token},function(res){
        $('#loader-box').hide();
        console.log(res);
        if(res.status==1){
            var rechargeMessage = res.data.active_list;
            if(rechargeMessage.length>0){
                $('.recharge-message-form').show();
                activeMessageLi+='<li><i class="icon-checked"></i><span>'+rechargeMessage[0].title +'</span> <span>截止时间：'+rechargeMessage[0].end_date+'</span></li>';
                if(rechargeMessage.length>1){
                    for(var a1=1;a1<rechargeMessage.length;a1++){
                        activeMessageLi+='<li><i class="icon-check"></i><span>'+rechargeMessage[a1].title +'</span> <span>截止时间：'+rechargeMessage[a1].end_date+'</span></li>';
                    }
                }
                activeMessageLi+='<li><i class="icon-check"></i><span>不参与活动</span></li>';
                activeMessageUl.html(activeMessageLi);
                chooseRechargeMessage(rechargeMessage);
            }else{
                $('.recharge-message-form').hide();
                rechargeActiveChoose = 0;
                rechargeActiveNum = [];
            }
        }else{
            $('.recharge-message-form').hide();
...
//选择充值活动
function chooseRechargeMessage(activeList){
    //默认选择第一个
    rechargeActiveChoose = parseInt(activeList[0].id);
    rechargeActiveNum = activeList[0].config_list;
    $('.recharge-message-form .main-form-item-r li').click(function(){
        var activeIndex = $(this).index();
        $(".recharge-message-form .main-form-item-r li i").attr("class",'icon-check');
        $(this).find('i').attr("class",'icon-checked animated bounceIn');
        if(activeIndex>=activeList.length){
            rechargeActiveChoose = 0;
            rechargeActiveNum = [];
        }else {
            rechargeActiveChoose = parseInt(activeList[activeIndex].id);
            rechargeActiveNum = activeList[activeIndex].config_list;
        }
    });
}
//根据金额显示充多少送多少活动
function rechargeActiveAccount(activeAccountList,tarInput){
    tarInput.parents('.main-form-item').find('.form-tips-right').hide();
    for(var k=0;k<activeAccountList.length;k++){
        if(Math.floor(tarInput.val()) >= Math.floor(activeAccountList[k][2])){
            tarInput.parents('.main-form-item').find('.form-tips-right').show();
            tarInput.parents('.main-form-item').find('.form-tips-right em').html(activeAccountList[k][3]);
        }
    }
}

/*****在线支付*****/
function onlinePayRecharge(){
    postInfo = {};
    //打开 关闭 支付类型框
    $('.online-pay .recharge-type-input').hover(function(){
        rechargeTypesBox1.show();
        $('.online-pay .recharge-type-input i').attr('class','icon-up');
    },function(){
        rechargeTypesBox1.hide();
        $('.online-pay .recharge-type-input i').attr('class','icon-down');
    });
    //选择微信 支付宝
    $('.online-pay .recharge-types .other-type-box li').click(function(){
        var typeIndex = $(this).index();
        $('.online-pay .recharge-types li span').removeClass('checked');
        $(this).find('span').addClass('checked');
        rechargeTypeInput1.val($(this).find('span').html());
        $('.online-pay .recharge-types').hide(60,function(){
            $('.online-pay .recharge-type-input i').attr('class','icon-down');
            //选择汇款类型提示
            formTips1.find('.min-pay').html('10');
            formTips1.find('.max-pay').html('3000');
            formTips1.show();
            //显示充值活动
            showRechargeMessage();
        });
    ...
    //选择银行
    $('.online-pay .recharge-types .bank-box li').click(function(){
        $('.online-pay .recharge-types li span').removeClass('checked');
        $(this).find('span').addClass('checked');
        rechargeTypeInput1.val($(this).find('span').html());
        $('.online-pay .recharge-types').hide(60,function(){
            ...
            //选择汇款类型提示
            formTips1.find('.min-pay').html('10');
            formTips1.find('.max-pay').html('5000');
            formTips1.show();
            //显示充值活动
            showRechargeMessage();
        });
    ...
    //点击去支付
    $('#onlinePay-recharge').click(function(){
        var onlinePay1 = $('#recharge-type1'),
            onlinePay2 = $('#recharge-accout1');
        if(onlinePay1.val()==''){
            alertMsg('请选择支付类型！');
            return
        }
        ...
        rechargeActiveAccount(rechargeActiveNum,onlinePay2);
        console.log(rechargeActiveChoose,rechargeActiveNum);
    })
}


/*****转账汇款*****/
function transferAccountsRecharge(){
    //打开 关闭 支付类型框
    $('.transfer-accounts .recharge-type-input').hover(function(){
        rechargeTypesBox2.show();
        $('.transfer-accounts .recharge-type-input i').attr('class','icon-up');
    },function(){
        rechargeTypesBox2.hide();
        $('.transfer-accounts .recharge-type-input i').attr('class','icon-down');
    });
    //银行
    showBankTransfer();
    //微信 支付宝
    chooseWeixinTransfer();
}

//显示汇款银行列表
function showBankTransfer(){
    postData('remit',{'token':localStorage.token,'stype':0},function(res){
        $('#loader-box').hide();
        console.log(res);
        var companyBankUl = $('.transfer-accounts .recharge-types .bank-box');
        var companyBankLi = '';
        if(res.status==1){
            var companyBankLists = res.data;
            if(companyBankLists.length>0){
                for(var a2=0;a2<companyBankLists.length;a2++){
                    companyBankLi+=' <li><span class="bank '+companyBankLists[a2].code+'">'+companyBankLists[a2].name+'</span></li>'
                }
                companyBankUl.html(companyBankLi);
                //选择银行
                chooseBankTransfer(companyBankLists);
            }
        }
    });
}
//选择银行
function chooseBankTransfer(list){
    $('.transfer-accounts .recharge-types .bank-box li').click(function(){
        clearInterval(amountDownTimer);
        postInfo = {};
        $('.transfer-accounts-weixin').hide();
        $('.weixin-step1').hide();
        $('.weixin-step2').hide();
        var _index = $(this).index();
        $('.transfer-accounts .recharge-types li span').removeClass('checked');
        $(this).find('span').addClass('checked');
        rechargeTypeInput2.val($(this).find('span').html());
        //关闭类型选择框
        rechargeTypesBox2.hide(60,function(){
            //向上图标变成向下
            $('.transfer-accounts .recharge-type-input i').attr('class','icon-down');
            //选择银行类型提示
            formTips2.find('.min-pay').html(list[_index].min_pay);
            formTips2.find('.max-pay').html(list[_index].max_pay);
            formTips2.show();
            //显示银行详情
            var randomNum = '';
            $('.transfer-accounts-bank').show(60,function(){
                $(this).find('.fl p').eq(0).find('span:nth-child(2)').html(list[_index].name);
                $(this).find('.fl p').eq(0).find('i').attr('data-clipboard-text',list[_index].name);
                $(this).find('.fl p').eq(1).find('span:nth-child(2)').html(list[_index].bank_username);
                $(this).find('.fl p').eq(1).find('i').attr('data-clipboard-text',list[_index].bank_username);
                $(this).find('.fl p').eq(2).find('span:nth-child(2)').html(list[_index].bank_account);
                $(this).find('.fl p').eq(2).find('i').attr('data-clipboard-text',list[_index].bank_account);
                $(this).find('.fl p').eq(3).find('span:nth-child(2)').html(list[_index].bank_info);
                $(this).find('.fl p').eq(3).find('i').attr('data-clipboard-text',list[_index].bank_info);
                $(this).find('.fl p').eq(4).find('span:nth-child(2)').html(Math.random().toString(36).substr(2,6));
                randomNum = $(this).find('.fl p').eq(4).find('span:nth-child(2)').html();
                $(this).find('.fl p').eq(4).find('i').attr('data-clipboard-text',randomNum);
                copeText($('#copy3'));
                copeText($('#copy4'));
                copeText($('#copy5'));
                copeText($('#copy6'));
            ...
            showRechargeMessage();
            //点击已汇款，确认提交
            $('#transfer-accounts-recharge').click(function(){
                var transferAccounts1 = $('#recharge-name2'),
                    transferAccounts2 = $('#recharge-accout2');
                if(transferAccounts1.val()==''){
                    alertMsg('请填写银行卡姓名！');
                    return
                }
                if(transferAccounts2.val()==''){
                    alertMsg('请填写实际汇款金额！');
                    return
                }
                rechargeActiveAccount(rechargeActiveNum,transferAccounts2);
                postInfo = {
                    'active_id':rechargeActiveChoose,
                    'recharge_name':transferAccounts1.val(),
                    'money':transferAccounts2.val(),
                    'card_id':list[_index].id,
                    'remark':randomNum,
                    'type':0
                };
                console.log(postInfo);
                postData('remit-operate',postInfo,function(res){
                    $('#loader-box').hide();
                    console.log(res);
                    alertMsg(res.msg,function(){
                        //$('.transfer-accounts-bank').hide();
                        //$('.transfer-accounts-weixin').hide();
                        //formTips2.hide();
                        //rechargeTypeInput2.val('');
                        transferAccounts1.val('');
                        transferAccounts2.val('');
                        if(res.status==1){
...
//选择微信（同支付宝）
function chooseWeixinTransfer(){
    $('.transfer-accounts .recharge-types .other-type-box li').click(function(){
        clearInterval(amountDownTimer);
        postInfo = {};
        var weixinTransferType = $(this).index()+1;
        $('.transfer-accounts-bank').hide();
        $('.transfer-accounts .recharge-types li span').removeClass('checked');
        $(this).find('span').addClass('checked');
        rechargeTypeInput2.val($(this).find('span').html());
        ...
        rechargeTypesBox2.hide(60,function(){
            ...
            $('.transfer-accounts .recharge-type-input i').attr('class','icon-down');
            //选择汇款类型提示
            formTips2.find('.min-pay').html('10');
            formTips2.find('.max-pay').html('3000');
            formTips2.show();
            //显示充值活动
            showRechargeMessage();
            //显示微信输入框
            $('.transfer-accounts-weixin').show(60,function(){
                $('.weixin-step1').show();
                $('.weixin-step2').hide();
                var weixinTransfer1 = $('#recharge-accout2-wx');
                weixinTransfer1.val('');
                //点击生成二维码
                $('#transfer-accounts-weixin1').click(function(){
                    if(weixinTransfer1.val()==''){
                        alertMsg('请填写汇款金额！');
                        return
                    }
                    rechargeActiveAccount(rechargeActiveNum,weixinTransfer1);
                    getWeixinErweima(weixinTransferType);
                })
            });
        });
    });
...
//倒计时
function amountDownFn(){
    $('#recharge-amount-down').html('05:00');
    var tarHtml = '';
    var leftTime=parseInt(5*60);//换算成秒
    amountDownTimer = setInterval(function(){
        leftTime--;
        var minute=parseInt(leftTime/60%60);
        var second=parseInt(leftTime%60);
        tarHtml=twoNum(minute)+":"+twoNum(second);
        if(minute==0 && second==0){
            tarHtml= '您已经超过支付时间';
            clearInterval(amountDownTimer);
        }
        $('#recharge-amount-down').html(tarHtml);
    },1000);
}
//显示二维码
function erweimaShow(type){
    postData('remit',{'token':localStorage.token,'stype':type},function(res){
        $('#loader-box').hide();
        console.log(res);
        if(res.status==1){
            if(res.data.length>0){
                $('#erweima-box img').attr('src',res.data[0].website);
            }
        }
    });
}
...
//选择微信（同支付宝）---生成二维码
function getWeixinErweima(typeIndex){
    $('.weixin-step1').hide();
    $('.weixin-step2').show(60,function(){
        //微信
        if(typeIndex==1){
            $('.weixin-detail-box .fl p').html('微信扫一扫');
            erweimaShow(1);
        }
        //支付宝
        if(typeIndex==2){
            $('.weixin-detail-box .fl p').html('支付宝扫一扫');
            erweimaShow(2);
        }
        //倒计时
        amountDownFn();
        //点击 已汇款，确认提交
        $('#transfer-accounts-weixin2').click(function(){
            postInfo = {
                'active_id':rechargeActiveChoose,
                'money':$('#recharge-accout2-wx').val(),
                'type':typeIndex
            };
            console.log(postInfo);
            postData('remit-operate',postInfo,function(res){
...
function addBank(){
    ...
    $('#add-bank-open').click(function(){
        $('#user-name').html(pangu.myData.name);
        openContent('添加银行卡',$('#add-bank'));
        var thisBankName = '',
            messageSpan = $('#bank-number-input').find('span'),
            bankCardInput = $("input[name^='bankCard-number']");
        bankCardInput.focus(function(){
            bankCardInput.val('');
            messageSpan.html('');
        });
        bankCardInput.blur(function(){
            if(bankCardInput.val() ==''){
                messageSpan.html('请输入银行卡号码！');
                return
            }
            var cardNoNew = bankCardInput.val().split(' ').join('');
            if (cardNoNew.toString().length < 15 || cardNoNew.toString().length > 19) {
                messageSpan.html('银行卡位数必须是15到19位');
                return;
            }
            for (var i = 0, len1 = bankcardList.length; i < len1; i++) {
                for (var j = 0, len2 = bankcardList[i].patterns.length; j < len2; j++) {
                    if (bankcardList[i].patterns[j].reg.test(cardNoNew)) {
                        thisBankName = bankcardList[i].bankName;
                    }
                }
            }
            if(thisBankName==''){
                messageSpan.html('找不到匹配的银行卡，请重新输入');
                return;
            }
            messageSpan.html(thisBankName);
        });
        $('#add-bank-submit').click(function(){
            postInfo ={};
            var phoneNumber = $("input[name^='phone-number']");
            var phoneNumberMsg = phoneNumber.next('span');
            phoneNumberMsg.html('');
            if(phoneNumber.val()==''){
                //alertMsg('手机号码不能为空！');
                phoneNumberMsg.html('手机号码不能为空！');
                return;
            }
            if(!/^1\d{10}$/.test(phoneNumber.val())){
                //alertMsg('请输入符合规范的手机号码！');
                phoneNumberMsg.html('请输入符合规范的手机号码！');
                phoneNumber.val('');
                return
            }
            console.log('提交');
...
function withdraw(){
    $('#withdraw-apply').click(function(){
        postInfo ={};
        var withdraw1 = $("input[name^='account']"),
            withdraw2 = $("input[name^='payword']");
        if(withdraw1.val() ==''){
            alertMsg('请输入提现金额！');
            return
        }
        ...
        postInfo ={
            'account':withdraw1.val(),
            'payword':withdraw2.val()
        };
        console.log(postInfo);
    ...
    var deleteList = null;
    $('.sure-delete-bank').click(function(){
        deleteList = $(this).index();
        askBox('确认要删除吗？','确定','取消',function(){
...
/********************账户明细***********************/
//选择查询类型
function detailsTypeSelect(){
    $('.drop-list ul li').click(function(){
        var dropListVal = $(this).find('a').text();
        $(this).parents('.show-drop-list').find('.drop-select').html(dropListVal);
    });
}
